...
func _ready() -> void :
  ...
  for i in menu_buttons:
    i.mouse_entered.connect(_process_button_anims.bind(i, true))
    i.mouse_exited.connect(_process_button_anims.bind(i, false))
...
func _process_button_anims(button: Button, entered: bool):
  if entered:

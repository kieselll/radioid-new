...
func show_popup(popup_type : popup_types):
	...
	var opacity_tween = create_tween().set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_CUBIC).set_parallel()
...
func _on_create_world_button_pressed():
	if GlobalSaver.get_saves_list().has(name_edit.text.to_snake_case()):
		...
		GlobalSaver.write_save(name_edit.text.to_snake_case(), name_edit.text, seed_edit.text.to_int() if not seed_edit.text.is_empty() else randi())
		SceneTransition.start_trans()
		await SceneTransition.done

...
func write_to_logs(sender : Node, log_string : String) -> void:
	_log_file.store_line(str(sender.get_path()) + " - " + log_string + ": " + Time.get_time_string_from_system() + ":" + str(int((Time.get_unix_time_from_system() - int(Time.get_unix_time_from_system()))*1000)))
	_log_file.flush()

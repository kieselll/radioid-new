...
func _tick() -> void:
	...
	_fps_counter.text = (
		" FPS: "
		+ str(floor(Engine.get_frames_per_second()))
		+ "\n MAX: "
		+ str(_recent_frames.max())
		+ "\n MIN: "
		+ str(_recent_frames.min())
		+ "\n AVG: "
		+ str(floor(average_frames))
		+ "\n FRAME_TIME: "
		+ str(snapped(1 / Performance.get_monitor(Performance.TIME_FPS) * 1000, 0.001))
		+ " ms"
	)
	_draw_calls.text = (
		" TOTAL_NODES: "
		...
		+ "\n OBJECTS DRAWN: "
		+ str(Performance.get_monitor(Performance.RENDER_TOTAL_OBJECTS_IN_FRAME))
		+ "\n DRAW_CALLS: "
		+ str(Performance.get_monitor(Performance.RENDER_TOTAL_DRAW_CALLS_IN_FRAME))
	)
	_memory.text = (
		" MEMORY: "
		+ str(snapped(Performance.get_monitor(Performance.MEMORY_STATIC) / 1024 / 1000, 0.001))
		+ " MB"
		+ "\n MEMORY_MAX: "
		+ str(snapped(Performance.get_monitor(Performance.MEMORY_STATIC_MAX) / 1024 / 1000, 0.001))
		+ " MB"
	)

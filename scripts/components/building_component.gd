...
func build(coords: Vector4i, id: int, time: float = 5):
	...
	progressbar.position = (GridUtils.chunk_coord_to_tile_coord(coords) - Vector2i(12, 26))
	_parent.get_node(GlobalRef.get_tilemap_layer_path(_data.layer)).add_child(progressbar)
	...
	_parent.get_node(GlobalRef.get_tilemap_layer_path(_data.queued_layer)).erase_cell(coords)

...
func get_custom_tile_data(param_name: String):
	if not custom_tile_data.keys().has(param_name):
		push_warning(
			(
				"Tried to get %s from tile at %s, but custom_tile_data doesn\'t have key %s"
				% [param_name, _local_pos, param_name]
			)
		)
		return null
...
func calculate_neighbor_overlays() -> void:
	...
	for i in _tilemap.get_surrounding_cells(_local_pos):
		if tile_coords_to_connect.has(i - _local_pos):
			...
			var neighbor_id: int = (
				neighbor_tile_data.get_custom_data("id") if neighbor_tile_data else 0
			)
...
func _draw() -> void:
	for i in _current_overlay_coords:
		...
		if not _connecting_terrains_dict.keys().has(terrain_id):
			continue

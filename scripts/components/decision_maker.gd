...
func calculate_action_priority_modifier(action_name : StringName, base_priority : int, location : Vector2i = Vector2i.MAX, emotion_modifier : float = 1) -> float:
	var skill_modified : float = 0
	var distance : float = _movement_component.get_local_position().distance_squared_to(location) if location != Vector2i.MAX else 0
	var skill_level : int
	var priority = 0 if action_name == &"wander_action" else base_priority
	print("\ndistance_from ", _movement_component.get_local_position()," to ", location, " is ", _movement_component.get_local_position().distance_squared_to(location))
	if _ability_manager.action_name_to_ability_name(action_name):
		skill_level = _ability_manager.get_ability_level(_ability_manager.action_name_to_ability_name(action_name))
...
func _queue_sort(a, b):
	if a.args.keys().has(&"target"):
		a.priority = calculate_action_priority_modifier(a.action_name, TEMP_BASE_PRIORITY, a.args[&"target"])
	else:
		a.priority = calculate_action_priority_modifier(a.action_name, TEMP_BASE_PRIORITY)
	if b.args.keys().has(&"target"):
		b.priority = calculate_action_priority_modifier(b.action_name, TEMP_BASE_PRIORITY, b.args[&"target"])
	else:

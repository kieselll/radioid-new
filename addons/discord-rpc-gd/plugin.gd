...
var restart_window: ConfirmationDialog = (
	preload("res://addons/discord-rpc-gd/restart_window.tscn").instantiate()
)
...
func _enter_tree() -> void:
	...
	get_editor_interface().get_editor_settings().settings_changed.connect(
...
func _ready() -> void:
	...
	if !get_editor_interface().get_editor_settings().has_setting(
		"DiscordRPC/EditorPresence/enabled"
	):
		get_editor_interface().get_editor_settings().set_setting(
			"DiscordRPC/EditorPresence/enabled",
			plugin_cfg.get_value("Discord", "editor_presence", false)
		)
...
func _enable_plugin() -> void:
	if FileAccess.file_exists(
		ProjectSettings.globalize_path("res://") + "addons/discord-rpc-gd/bin/.gdignore"
	):
...
func _on_editor_settings_changed() -> void:
	plugin_cfg.set_value(
		"Discord",
		"editor_presence",
		get_editor_interface().get_editor_settings().get_setting(
			"DiscordRPC/EditorPresence/enabled"
		)
	)
	...
	if (
		get_editor_interface().get_editor_settings().has_setting(
			"DiscordRPC/EditorPresence/enabled"
		)
		&& get_editor_interface().get_editor_settings().get_setting(
			"DiscordRPC/EditorPresence/enabled"
